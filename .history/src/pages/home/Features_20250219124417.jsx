...
function FeatureCard(props){
  ...
  const {isMobile} = useContext(MyContext)
  return (
    <div style={{width:isMobile?"60%":320,height:isMobile?400:350}} 
    className={`card ${props.BGcolorClass=="bg-white" ? "text-dark" : "text-white"} border-0 shadow-md pt-5 pb-3 px-3 ${props.BGcolorClass} ${isMobile&&"px-5"}`}>
      ...
      <h5 className='fw-bold mt-3 company-text' style={{fontSize:isMobile?40:22}}>{props.title}</h5>
      <br />
      <p style={{fontSize:isMobile?30:15, lineHeight:"31px"}}>
        {props.content}
      </p>
      <div>
        <a href="#" style={{fontSize:isMobile?20:15}}
